...
def auth_before(req, session):
    ...
    if req.url.path in ["/admin/login", "/admin/logout"]:
        return
                ...
                Ul(
                    Li(
                        A(
                            "Dashboard",
                            href="/admin",
                            cls="active" if active_tab == "dashboard" else "",
                        )
                    ),
                    Li(
                        A(
                            "Logs",
                            href="/admin/logs",
                            cls="active" if active_tab == "logs" else "",
                        )
                    ),
                    Li(
                        A(
                            "Keys",
                            href="/admin/keys",
                            cls="active" if active_tab == "keys" else "",
                        )
                    ),
    ...
    return Layout(
        ...
        Div(
            ...
            Div(
                ...
                Div(
                    f"{int(stats['success_calls'] / stats['total_calls'] * 100) if stats['total_calls'] else 0}%",
                    cls="stat-value text-secondary",
                ),
                cls="stat",
            ),
            Div(
...
async def logs():
    ...
    rows = [
        Tr(
            ...
            Td(
                Div(
                    cls=f"badge {'badge-success' if log.success else 'badge-error'}",
                    content="OK" if log.success else "ERR",
                )
            ),
