...
async def test_zrelay_service_call_success():
    ...
    mock_adapter.invoke = AsyncMock(return_value=ToolResult(
        ok=True, tool="test-tool", operation="op1", request_id="req-1", data={"res": "ok"}
    ))
    ...
    result = await service.call_tool(
        tool_name="test-tool", operation="op1", payload={}, 
        client_key_id="key-1", request_id="req-1", route="/test"
    )
...
async def test_zrelay_service_tool_not_found():
    ...
    result = await service.call_tool(
        tool_name="unknown", operation="op1", payload={}, 
        client_key_id="key-1", request_id="req-1", route="/test"
    )
